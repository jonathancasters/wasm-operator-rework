...
def main():
    ...
    manifest_script = os.path.join(args.script_dir, "operators/rust-operator-bare-metal/generate-manifests.sh")
    ...
    for i in range(1, args.operator_count + 1):
        deployment_name = f"ring-operator-bare-metal-{i}"
    ...
    active_memory = get_memory_usage("ring-operator-bare-metal")
    ...
    idle_memory = get_memory_usage("ring-operator-bare-metal")
